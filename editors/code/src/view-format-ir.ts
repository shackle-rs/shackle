...
const viewFormatIrRequest = new RequestType<
	TextDocumentPositionParams,
	string,
	void
>("shackle-ls/viewFormatIr")

class FormatIrProvider implements TextDocumentContentProvider {
	readonly eventEmitter = new EventEmitter<Uri>()
	readonly uri: Uri = Uri.parse("shackle-ls://viewFormatIr/FormatIr.txt")
	...
	provideTextDocumentContent(
		...
		return this.client.sendRequest(viewFormatIrRequest, params, ct)
	}
...
let provider: FormatIrProvider | null = null
export async function handleFormatIrViewCommand(client: LanguageClient) {
	if (!provider) {
		provider = new FormatIrProvider(client, () => {
			provider = null
		})
	}
