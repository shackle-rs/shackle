	...
	fn collect_expression_inner(&mut self, idx: shackle_hir::ExpressionId<'db>) -> Expression<'db> {
		...
		let mut result = match &self.data[idx] {
			...
			shackle_hir::Expression::Call(c) => {
				...
				let join_class: Option<PatternRef<'db>> =
					if is_equality_op && c.arguments.len() == 2 {
						...
						let join = if all_class && distinct {
							Ty::most_specific_supertype(
								eq_db,
								c.arguments.iter().map(|arg| self.types[*arg]),
							)
							...
							c.arguments
								...
								.all(|(arg, sc)| match sc {
									Some(source_class) if source_class != jc => {
										...
										is_root
											|| self
												.reference_projection_join_constructor(
													*source_class,
													*jc,
												)
												.is_some()
				...
				// domains.
				let ordinal_compare =
					join_class.is_none() && is_equality_op && c.arguments.len() == 2 && {
						let classes = c
							.arguments
							.iter()
							.map(|arg| self.types[*arg].class_type(eq_db).map(class_pattern))
								...
								match self.types.name_resolution(*arg).and_then(|res| {
									...
									None => {
										let ct = self
											...
											.expect(
												"join filter guarantees a projectable reference",
											);
										self.project_reference_identity(
											expr, join_class, ct, arg_origin,
										)
									}
								}
							}
				...
				let needs_redispatch = match &function {
					Callable::Function(f) => {
						...
						params.len() != arguments.len()
							|| !arguments.iter().zip(params.iter()).all(|(arg, p)| {
								arg.ty().is_subtype_of(db, self.parent.model[*p].ty())
							})
					}
					_ => false,
				};
			...
			shackle_hir::Expression::Identifier(_) => {
					...
					(expr.ty().class_type(db), ty.class_type(db))
				{
						...
						&& expr.ty().is_subtype_of(db, ty)
					{
						...
						let relabel_ty =
							self.parent.substitute_class_with_potential_enum(expr.ty());
						let mut relabeled =
