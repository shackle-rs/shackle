...
// SAFETY: Expression<'db, T> contains only owned data — no raw references tied to the
...
// the Rust trait solver to hang (Expression -> ExpressionData -> Vec<Expression> -> cycle).
unsafe impl<'db, T: Marker> salsa::SalsaValue for Expression<'db, T> {}
...
/// Array literal
#[derive(Clone, Debug, Default, Hash, PartialEq, Eq, salsa::SalsaValue)]
