...
pub struct ReplacementMap<'db, Dst: Marker, Src: Marker = ()> {
	annotations: FxHashMap<AnnotationId<'db, Src>, AnnotationId<'db, Dst>>,
	constraints: FxHashMap<ConstraintId<'db, Src>, ConstraintId<'db, Dst>>,
	declarations: FxHashMap<DeclarationId<'db, Src>, DeclarationId<'db, Dst>>,
	enumerations: FxHashMap<EnumerationId<'db, Src>, EnumerationId<'db, Dst>>,
	functions: FxHashMap<FunctionId<'db, Src>, FunctionId<'db, Dst>>,
	outputs: FxHashMap<OutputId<'db, Src>, OutputId<'db, Dst>>,
}

impl<'db, Src: Marker, Dst: Marker> ReplacementMap<'db, Dst, Src> {
	/// Get the replacement for an item
	pub fn get_item(&self, src: ItemId<'db, Src>) -> Option<ItemId<'db, Dst>> {
		match src {
	...
	pub fn get_annotation(&self, src: AnnotationId<'db, Src>) -> Option<AnnotationId<'db, Dst>> {
		self.annotations.get(&src).copied()
	}

	/// Insert an annotation ID into the replace map
	pub fn insert_annotation(&mut self, src: AnnotationId<'db, Src>, dst: AnnotationId<'db, Dst>) {
		let _ = self.annotations.insert(src, dst);
	}

	/// Get the replacement for this constraint ID if any
	pub fn get_constraint(&self, src: ConstraintId<'db, Src>) -> Option<ConstraintId<'db, Dst>> {
		self.constraints.get(&src).copied()
	}

	/// Insert an constraint ID into the replace map
	pub fn insert_constraint(&mut self, src: ConstraintId<'db, Src>, dst: ConstraintId<'db, Dst>) {
		let _ = self.constraints.insert(src, dst);
	}

	/// Get the replacement for this declaration ID if any
	pub fn get_declaration(&self, src: DeclarationId<'db, Src>) -> Option<DeclarationId<'db, Dst>> {
	...
	pub fn insert_declaration(
		&mut self,
		src: DeclarationId<'db, Src>,
		dst: DeclarationId<'db, Dst>,
	) {
		let _ = self.declarations.insert(src, dst);
	}

	/// Get the replacement for this enumeration ID if any
	pub fn get_enumeration(&self, src: EnumerationId<'db, Src>) -> Option<EnumerationId<'db, Dst>> {
		self.enumerations.get(&src).copied()
	}

	/// Insert an enumeration ID into the replace map
	pub fn insert_enumeration(
		&mut self,
		src: EnumerationId<'db, Src>,
		dst: EnumerationId<'db, Dst>,
	) {
		let _ = self.enumerations.insert(src, dst);
	}

	/// Get the replacement for this enum member ID if any
	pub fn get_enum_member(&self, src: EnumMemberId<'db, Src>) -> Option<EnumMemberId<'db, Dst>> {
	...
	pub fn get_function(&self, src: FunctionId<'db, Src>) -> Option<FunctionId<'db, Dst>> {
		self.functions.get(&src).copied()
	}

	/// Insert an function ID into the replace map
	pub fn insert_function(&mut self, src: FunctionId<'db, Src>, dst: FunctionId<'db, Dst>) {
		let _ = self.functions.insert(src, dst);
	}

	/// Get the replacement for this output ID if any
	pub fn get_output(&self, src: OutputId<'db, Src>) -> Option<OutputId<'db, Dst>> {
		self.outputs.get(&src).copied()
	}

	/// Insert an output ID into the replace map
	pub fn insert_output(&mut self, src: OutputId<'db, Src>, dst: OutputId<'db, Dst>) {
		let _ = self.outputs.insert(src, dst);
	}
}

/// Trait for folding a model, and adding a transformed version of the items into another model.
pub trait Folder<'a, 'db, Dst: Marker, Src: Marker = ()> {
	...
	fn add_item(&mut self, db: &'db dyn Db, model: &'a Model<'db, Src>, item: ItemId<'db, Src>) {
	...
	fn add_solve(&mut self, db: &'db dyn Db, model: &'a Model<'db, Src>) {
	...
	fn fold_identifier(
		...
		db: &'db dyn Db,
		model: &'a Model<'db, Src>,
		identifier: &'a ResolvedIdentifier<'db, Src>,
	...
	fn fold_record_literal(
		...
		db: &'db dyn Db,
		model: &'a Model<'db, Src>,
		rl: &'a RecordLiteral<'db, Src>,
	) -> RecordLiteral<'db, Dst> {
	...
	fn fold_set_comprehension(
		...
		db: &'db dyn Db,
		model: &'a Model<'db, Src>,
		c: &'a SetComprehension<'db, Src>,
	) -> SetComprehension<'db, Dst> {
	...
	fn fold_pattern(
		...
		db: &'db dyn Db,
		model: &'a Model<'db, Src>,
		pattern: &'a Pattern<'db, Src>,
	) -> Pattern<'db, Dst> {
		fold_pattern(self, db, model, pattern)
	}
}

fn alloc_expression<'a, 'db: 'a, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	db: &'db dyn Db,
	origin: impl Into<Origin<'db>>,
	value: impl ExpressionBuilder<'db, U>,
	folder: &mut F,
) -> Expression<'db, U> {
...
pub fn add_model<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
) {
...
/// Fold an item
pub fn add_item<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	item: ItemId<'db, T>,
...
/// Add the folded version of this annotation item into the destination model.
pub fn add_annotation<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	a: AnnotationId<'db, T>,
...
/// Fold an annotation item
pub fn fold_annotation<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	a: &'a Annotation<'db, T>,
...
/// Add the folded version of this constraint item into the destination model.
pub fn add_constraint<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: ConstraintId<'db, T>,
...
/// Fold a constraint item
pub fn fold_constraint<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a Constraint<'db, T>,
...
/// Add the folded version of this declaration item into the destination model.
pub fn add_declaration<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a declaration item
pub fn fold_declaration<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	d: &'a Declaration<'db, T>,
) -> Declaration<'db, U> {
	let mut declaration =
...
/// Fold a declaration ID
pub fn fold_declaration_id<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	d: DeclarationId<'db, T>,
...
/// Add the folded version of this enumeration item into the destination model.
pub fn add_enumeration<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold an enumeration item
pub fn fold_enumeration<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	e: &'a Enumeration<'db, T>,
) -> Enumeration<'db, U> {
...
/// Fold an enumeration ID
pub fn fold_enumeration_id<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	e: EnumerationId<'db, T>,
...
/// Fold an enum member ID
pub fn fold_enum_member_id<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	e: EnumMemberId<'db, T>,
...
/// Fold a constructor
pub fn fold_constructor<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a Constructor<'db, T>,
) -> Constructor<'db, U> {
	Constructor {
		name: c.name,
...
/// Add the folded version of this function item into the destination model.
pub fn add_function<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	f: FunctionId<'db, T>,
...
/// Note that this doesn't fold the body, which has to be processed at the end
/// since it may refer to items which have not been added yet.
pub fn fold_function<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	f: &'a Function<'db, T>,
) -> Function<'db, U> {
	let mut function: Function<'db, U> =
		Function::new(f.name(), folder.fold_domain(db, model, f.domain()));
...
/// Fold an function ID
pub fn fold_function_id<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	f: FunctionId<'db, T>,
...
/// have been added, since they can refer to items which have not been added to
/// the destination model yet.
pub fn fold_function_body<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	f: FunctionId<'db, T>,
...
/// Add the folded version of this output item into the destination model.
pub fn add_output<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	o: OutputId<'db, T>,
...
/// Fold an output item
pub fn fold_output<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	o: &'a Output<'db, T>,
...
/// Fold an output ID
pub fn fold_output_id<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	o: OutputId<'db, T>,
...
/// Add the folded version of the solve item into the destination model.
pub fn add_solve<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
) {
...
/// Fold the solve item
pub fn fold_solve<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	s: &'a Solve<'db, T>,
...
/// Fold an identifier
pub fn fold_identifier<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	identifier: &'a ResolvedIdentifier<'db, T>,
) -> ResolvedIdentifier<'db, U> {
	match identifier {
...
/// Fold a callable
pub fn fold_callable<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	function: &'a Callable<'db, T>,
) -> Callable<'db, U> {
	match function {
...
/// Fold an array literal
pub fn fold_array_literal<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a set literal
pub fn fold_set_literal<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a tuple literal
pub fn fold_tuple_literal<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a record literal
pub fn fold_record_literal<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold an array comprehension
pub fn fold_array_comprehension<
	'a,
	'db,
	T: Marker,
	U: Marker,
	F: Folder<'a, 'db, U, T> + ?Sized,
>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a ArrayComprehension<'db, T>,
) -> ArrayComprehension<'db, U> {
	ArrayComprehension {
		generators: c
			.generators
			.iter()
...
/// Fold a set comprehension
pub fn fold_set_comprehension<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a SetComprehension<'db, T>,
) -> SetComprehension<'db, U> {
	SetComprehension {
		generators: c
			.generators
			.iter()
...
pub fn fold_tuple_access<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
pub fn fold_record_access<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold an if-then-else expression
pub fn fold_if_then_else<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a case expression
pub fn fold_case<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a Case<'db, T>,
...
/// Fold a call expression
pub fn fold_call<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	c: &'a Call<'db, T>,
...
/// Fold a let expression
pub fn fold_let<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	l: &'a Let<'db, T>,
...
/// Fold a lambda expression
pub fn fold_lambda<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
	l: &'a Lambda<'db, T>,
...
pub fn fold_expression<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a generator
pub fn fold_generator<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a domain
pub fn fold_domain<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
...
/// Fold a pattern
pub fn fold_pattern<'a, 'db, T: Marker, U: Marker, F: Folder<'a, 'db, U, T> + ?Sized>(
	folder: &mut F,
	db: &'db dyn Db,
	model: &'a Model<'db, T>,
