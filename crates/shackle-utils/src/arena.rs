...
pub struct ArenaIndex<T> {
	/// The 1-based index into the arena (zero is not used so that Option<ArenaIndex<T>> is the same size
	index: NonZeroU32,
	/// Phantom data to associate this index with the type of arena it belongs to
...
impl<T: Debug> Debug for Arena<T> {
	fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
		fmt.debug_struct("Arena")
			.field("len", &self.len())
			.field("data", &ArenaDebug(&self.items[..]))
			.finish()
	}
}

/// Helper for debug printing an arena
struct ArenaDebug<'a, T>(&'a [T]);

impl<T: Debug> Debug for ArenaDebug<'_, T> {
	fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		let items = self
			...
			.iter()
			.enumerate()
			.map(|(i, v)| (ArenaIndex::<T>::new((i + 1) as u32), v));
		for (k, v) in items {
			let _ = dbg.entry(&k, &v);
		}
		dbg.finish()
...
impl<T> Arena<T> {
	...
	pub fn keys(&self) -> impl DoubleEndedIterator<Item = ArenaIndex<T>> {
	...
	pub fn iter(&self) -> impl DoubleEndedIterator<Item = (ArenaIndex<T>, &T)> {
		self.items
			.iter()
			.enumerate()
	...
	/// Get an iterator of pairs of `ArenaIndex` and values allocated in this arena.
	pub fn iter_mut(&mut self) -> impl DoubleEndedIterator<Item = (ArenaIndex<T>, &mut T)> {
		self.items
			.iter_mut()
			.enumerate()
			.map(|(idx, o)| (ArenaIndex::new((idx + 1) as u32), o))
...
pub struct ArenaMap<K, V> {
	...
	items: Vec<Option<V>>,
	/// Phantom data to associate this map with the type of data
	phantom: PhantomData<K>,
}

impl<K, V: Debug> Debug for ArenaMap<K, V> {
	fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
		let mut dbg = fmt.debug_map();
...
impl<K, V> IntoIterator for ArenaMap<K, V> {
	type IntoIter = std::iter::FilterMap<
		std::iter::Enumerate<std::vec::IntoIter<Option<V>>>,
		fn((usize, Option<V>)) -> Option<(ArenaIndex<K>, V)>,
	>;
	type Item = (ArenaIndex<K>, V);
