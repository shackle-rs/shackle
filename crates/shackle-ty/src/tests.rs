...
type_registry!(
	...
	ty_var: Ty::type_inst_var(db, TyVar {
		ty_var: TyVarRef::new(InternedString::new(db, "$T")),
		enumerable: false,
		indexable: false,
		varifiable: false
	}),
...
fn test_record() {
	...
	assert_eq!(
		...
		&TyData::Record(
			OptType::NonOpt,
			Box::new([(InternedString::new(db, "a"), types.var_int),])
		)
	);
	...
	assert_eq!(
		...
		&TyData::Record(
			OptType::NonOpt,
			Box::new([
				(InternedString::new(db, "a"), types.par_int),
				(InternedString::new(db, "b"), types.par_bool)
			])
		)
	);
